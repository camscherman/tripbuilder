...
function kindRequest(arrMakerFn, noOfRequestsPerBatch, depCode, arrCode, startDate, endDate, incrementStep =1 ){
    ...
    const recRequest = (requests, noOfRequestsPerBatch, start) =>{
      ...
      // kindRequest(makeRequestArrayOneWay, 30, 'YVR', 'ICN', '2017-12-08', '2018-5-18', 8 )
...
// async function queryNarrowParameters(datesFn, depCode, arrCode, noOfWeeks){
...
//
//     const getApiResults =   (reqBodies,index) => {
//         console.log("ReqBodies", reqBodies)
//         if(index === reqBodies.length -1){
//             const promises = R.map(makeRequest, reqBodies[index])
...
//                                     const fpRS = R.path(["FlightResponse","FpSearch_AirLowFaresRS"], parsedBody)
//                                     // console.log("This isfpRS", fpRS)
//                                     if(fpRS === undefined){
//                                       console.log('nothing here')
//                                     } else {
//                                     const dataObj = DataParser.parsePrices(parsedBody)
//                                     parsedPrices.push(dataObj)
//                                     let lowestPrice = 5000
//                                     for(let obj of dataObj){
//                                       if(obj.adultPrice < lowestPrice){
//                                         lowestPrice = obj.adultPrice
//                                       }
//                                     }
//                                     console.log("Lowest price",lowestPrice )
//                                     // console.log("DATA FROM RESPONSE",DataParser.parsePrices(parsedBody))
//
//                                     arrayOfPushes.push(pushToDb(parsedBody))
//                                   }
//                                 }
//
//                                 console.log("Ending")
//                                 // return Promise.all(arrayOfPushes)
//
//              })
//         } else {
//           const promises = R.map(makeRequest, reqBodies[index])
...
//                               console.log("Ending")
//                               // return Promise.all(arrayOfPushes)
//                               return getApiResults(reqBodies, index+1)
//            })
//
//     }
// }
// getApiResults(await requests, 0)
// debugger
// return parsedPrices
// }
...
function getSixMonthsData(depCode, arrCode){
  const whereParams = {
    'depCode': depCode,
    'arrCode': arrCode
  }
...
function getAverageMinimum(depCode, arrCode){
  getSixMonthsData(depCode, arrCode).then(res =>{
    const length = res.length
    res.reduc((item))
  })

}
getSixMonthsData('YVR', 'BKK').then( res => console.log(res))
...
module.exports = {kindRequest: kindRequest,
